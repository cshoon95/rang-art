...
const RegisterTable = React.memo(
  ({
    data,
    monthTotals,
    grandTotal,
    ...
    return (
      <TableContainer>
        <Table $isCurrentView={isCurrentView}>
          <Thead>
            <tr style={{ height: "50px" }}>
              <StickyThLeft>이름</StickyThLeft>
              {displayedMonths.map((m: string) => (
...
export default function RegisterClient({ academyCode }: Props) {
  ...
  const filteredData = useMemo(() => {
    ...
    return reportData.filter(
      (item: any) =>
        ...
        extractInitialConsonants(item.name).includes(debouncedSearch)
    );
  ...
  // 5. 표시할 월 목록 (전체 vs 이번달)
  const displayedMonths = useMemo(() => {
    return viewMode === "current" ? [currentMonth] : MONTHS;
  ...
  const handleNameClick = useCallback(
    ...
    [openModal, academyCode, year]
  );

  const handleYearChange = useCallback((diff: number) => {
  ...
  }, [filteredData, monthTotals, grandTotal, year]);
  ...
  return (
    <Container>
      ...
      <RegisterTable
        ...
        isCurrentView={viewMode === "current"}
      />
